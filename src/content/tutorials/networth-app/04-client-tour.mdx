...
Backend done, numbers trustworthy — now the face. The naive client fetches per page and the screens disagree with each other; what breaks is the user's trust in every number at once. This part tours fifteen pages fed from one shared state, then Part 5 removes the server entirely and watches what survives.
