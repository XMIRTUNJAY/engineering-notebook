...
The hub's audit found money math with no test harness — formulas nobody could check. The naive fix is more careful code in the same scattered places; what breaks is trust, because a wrong number looks exactly like a right one. This part builds the alternative: one folder where every number is checkable.
